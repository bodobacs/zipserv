...
int chm_list_start(struct chmFile *h,
					struct chm_list_rundata *rundata,
                 	 int what)
{
	memset(rundata, 0, sizeof(struct chm_list_rundata));	

    /* buffer to hold whatever page we're looking at */
    /* RWE 6/12/2003 */
    rundata->page_buf = malloc((unsigned int)h->block_len);

    rundata->type_bits = (what & 0x7);
    rundata->filter_bits = (what & 0xF8);

    if (rundata->page_buf == NULL)
        return 0;

    /* starting page */
    rundata->curPage = h->index_head;
...
{
    /* until we have either returned or given up */
    while(rundata->curPage != -1)
    {
		if(0 == rundata->inner_loop_on)
		{
			/* try to fetch the index page */
			if (_chm_fetch_bytes(h, rundata->page_buf, (UInt64)h->dir_offset + (UInt64)rundata->curPage * h->block_len, h->block_len) != h->block_len)
			{
				free(rundata->page_buf);
				return 0;
			}

			/* figure out start and end for this page */
			rundata->cur = rundata->page_buf;
			rundata->lenRemain = _CHM_PMGL_LEN;
			if (! _unmarshal_pmgl_header(&(rundata->cur), &(rundata->lenRemain), &(rundata->header)))
			{
				free(rundata->page_buf);
				return 0;
			}
			rundata->end = rundata->page_buf + h->block_len - (rundata->header.free_space);

			rundata->inner_loop_on = 1;
		}
        /* loop over this page */
        while(rundata->cur < rundata->end)
        {
            rundata->ui.flags = 0;

            if (! _chm_parse_PMGL_entry(&(rundata->cur), &(rundata->ui)))
            {
                free(rundata->page_buf);
                return 0;
            }
            /* get the length of the path */
            rundata->ui_path_len = strlen(rundata->ui.path)-1;

            /* check for DIRS */
            if (rundata->ui.path[rundata->ui_path_len] == '/')
                rundata->ui.flags |= CHM_ENUMERATE_DIRS;

            /* check for FILES */
            if (rundata->ui.path[rundata->ui_path_len] != '/')
                rundata->ui.flags |= CHM_ENUMERATE_FILES;

            /* check for NORMAL vs. META */
            if (rundata->ui.path[0] == '/')
            {

                /* check for NORMAL vs. SPECIAL */
                if (rundata->ui.path[1] == '#'  ||  rundata->ui.path[1] == '$')
                    rundata->ui.flags |= CHM_ENUMERATE_SPECIAL;
                else
                    rundata->ui.flags |= CHM_ENUMERATE_NORMAL;
            }
            else
                rundata->ui.flags |= CHM_ENUMERATE_META;

            if ((rundata->type_bits & rundata->ui.flags) 
			|| (rundata->filter_bits && (rundata->filter_bits & rundata->ui.flags)))
				{
		//			fprintf(stderr, "%s\n", rundata->ui.path);
					return 1;
				}
		...
		rundata->inner_loop_on = 0;

        /* advance to next page */
        rundata->curPage = rundata->header.block_next;
    }
